...
<head>
  ...
  if (isset($_POST['Saved'])) 
  {
    ...
    move_uploaded_file($_FILES["inPic"]["name"], "photo/".$_FILES["inPic"]["name"]);
    if ($con->query("insert into categories (name,pic) value ('$_POST[inName]','$filename')")) 
    {
      $h ="<div class='alert alert-success'>Successfully Saved :)</div>";
    }
    else
      {
        $h ="<div class='alert alert-danger'>Not saved Error : ".$con->error."</div>";
      }
  }
...
<div class="account" style="display: none;">
  ...
  <?php 
    ...
    ?>
    <br>
    <table class="table table-striped " style="width: 55%;margin: auto;">
      <tr>
        <th>No</th>
        <th>Name</th>
        <th>Items</th>
    ...
    {
      $i++;
      $array2 = $con->query("select count(*) as qty from inventeries where catId = '$row[id]'");  // give total items in a Category
  ...
  ?>
    <tr>
      ...
      <td><a href="delete.php?category=<?php echo $row['id'] ?>"><button class="btn btn-xs btn-danger">Remove</button></a></td>
    </tr>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title center">Add New Category</h4>
      </div>
